# Hello, I'm Himus :)

I am a Computer Science student focused on understanding how complex systems work by building them from the ground up. I value clean code, continuous learning, and I am currently looking for my first professional steps in the **Cybersecurity** field.

## About Me
* **Focus:** C++ development and low-level system understanding.
* **Approach:** I believe in learning by doing. I build projects from scratch to grasp the inner workings of software architecture.
* **Goal:** Transitioning my skills into the Cybersecurity domain.

## Main Project: InsulinMate
Originally, I developed **InsulinMate** in Python as a personal tool to manage my basal insulin doses. It turned out to be a common challenge for many people with Type 1 Diabetes, making it a project that combines technical problem-solving with real-world impact.

## Tech Stack
* **Languages:** C++, Java, Python, SQL
* **Tools:** Docker

## GitHub Stats
<p align="left">
  <img height="150" src="https://github-readme-stats.vercel.app/api?username=himusuwu&show_icons=true&theme=graywhite&hide_border=true" />
  <img height="150" src="https://github-readme-stats.vercel.app/api/top-langs/?username=himusuwu&layout=compact&theme=graywhite&hide_border=true" />
</p>

## Contact
* [LinkedIn](https://linkedin.com/in/yourusername)
* [Email](mailto:your.email@example.com)
